...
<div class="auth-card" style="max-width:600px">
    <div style="margin-bottom:24px">
        <h2 style="font-size:22px;font-weight:800;color:#0b044d;margin:0 0 6px">Edit Holiday</h2>
        <p style="font-size:13px;color:#6b6a8a;margin:0">Update holiday information</p>
    </div>

    <form action="{{ route('holidays.update', $holiday) }}" method="post" class="auth-form">
        @csrf
        @method('PUT')

        <div class="auth-field">
            <label>Holiday Name <span style="color:#dc2626">*</span></label>
            <input type="text" name="name" value="{{ old('name', $holiday->name) }}" placeholder="e.g. Independence Day" required>
        </div>

        <div class="auth-row-2">
            <div class="auth-field">
                <label>Start Date <span style="color:#dc2626">*</span></label>
                <input type="date" name="start_date" value="{{ old('start_date', $holiday->start_date) }}" required>
            </div>
            <div class="auth-field">
                <label>End Date <span style="color:#dc2626">*</span></label>
                <input type="date" name="end_date" value="{{ old('end_date', $holiday->end_date) }}" required>
            </div>
        </div>

        <div class="auth-field">
            <label>Duration (days) <span style="color:#dc2626">*</span></label>
            <input type="number" name="holiday_duration" value="{{ old('holiday_duration', $holiday->holiday_duration) }}" min="1" required>
        </div>
