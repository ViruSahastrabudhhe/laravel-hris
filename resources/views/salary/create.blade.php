...
<div class="auth-card" style="max-width:600px">
    <div style="margin-bottom:24px">
        <h2 style="font-size:22px;font-weight:800;color:#0b044d;margin:0 0 6px">Add New Salary</h2>
        <p style="font-size:13px;color:#6b6a8a;margin:0">Create a new salary record for an employee</p>
    </div>

    <form action="{{ route('salaries.store') }}" method="post" class="auth-form">
        @csrf
        ...
        <div class="auth-row-2">
            <div class="auth-field">
                ...
                <input type="number" name="salary_grade" value="{{ old('salary_grade') }}" placeholder="e.g. 11">
            </div>
            <div class="auth-field">
                <label>Step</label>
                <input type="number" name="step" value="{{ old('step', 1) }}" placeholder="e.g. 1" min="1">
            </div>
        </div>

        <div class="auth-field">
            <label>Amount <span style="color:#dc2626">*</span></label>
            <input type="number" name="amount" value="{{ old('amount') }}" placeholder="e.g. 25000" step="0.01" required>
        </div>
