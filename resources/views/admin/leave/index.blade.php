            ...
            <table class="payroll-table" id="benefits-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <div class="emp-cell">
                                <div class="emp-avatar" style="background:{{ ['#0b044d','#8e1e18','#15803d','#a16207','#7c3aed'][($employee->id % 5)] }}">{{ $employeeAvatar }}</div>
                                <div>
                        ...
                        @php
                            ...
                            foreach($employee->employeeCompensation as $compensation) {
                                if($compensation->compensation->id == 1) {
                                    $gsis = $compensation->amount;
                                    break;
                                }
                            }
                            $philhealth = 0;
                            foreach($employee->employeeCompensation as $compensation) {
                                if($compensation->compensation->id == 2) {
                                    $philhealth = $compensation->amount;
                                    break;
                                }
                            }
                            $pagibig = 0;
                            foreach($employee->employeeCompensation as $compensation) {
                                if($compensation->compensation->id == 3) {
                                    $pagibig = $compensation->amount;
                                    break;
                                }
                            }
                        @endphp
                        <td class="deduction" style="font-weight: 600;">₱{{ number_format($gsis, 2) }}</td>
                        <td class="deduction" style="font-weight: 600;">₱{{ number_format($philhealth, 2) }}</td>
                        <td class="deduction" style="font-weight: 600;">₱{{ number_format($pagibig, 2) }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
<script>
    $(function () {
        const leaveTable = $('#leaves-table').DataTable({
