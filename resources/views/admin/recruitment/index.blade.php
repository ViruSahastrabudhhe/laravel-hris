...
<div class="welcome-banner">
    <div class="banner-left">
        ...
        <div>
            ...
            <p>{{ config('app.carbon_date') }} &nbsp;·&nbsp; Job Postings & Applications</p>
        </div>
    </div>
    <div class="banner-right">
        <div class="recruit-search-wrap">
            ...
            <input type="text" id="search-input" placeholder="Search job postings..." class="recruit-search" oninput="filterJobs()">
        </div>
    </div>
</div>
...
<div class="stats-grid stats-grid-4" style="margin-bottom:24px">
    <div class="stat-card">
        ...
        <div class="stat-footer"><span class="stat-dot" style="background:#0b044d"></span><p class="stat-sub">All positions</p></div>
    </div>
    <div class="stat-card">
        <div class="stat-top">
        ...
        <p class="stat-value">{{ $openPositions }}</p>
        <div class="stat-footer"><span class="stat-dot" style="background:#15803d"></span><p class="stat-sub">Currently accepting</p></div>
    </div>
    <div class="stat-card">
        <div class="stat-top">
        ...
        <div class="stat-footer"><span class="stat-dot" style="background:#d9bb00"></span><p class="stat-sub">All applications</p></div>
    </div>
    <div class="stat-card">
        <div class="stat-top">
        ...
        <p class="stat-value">{{ $totalSlots }}</p>
        <div class="stat-footer"><span class="stat-dot" style="background:#8e1e18"></span><p class="stat-sub">Positions to fill</p></div>
    </div>
</div>
...
<div class="table-section recruitment-header" style="margin-bottom:16px">
    <div class="table-header">
        <div>
            ...
            <p class="table-sub">Municipality of Pagsanjan &nbsp;·&nbsp; <span id="showing-count">{{ $totalJobs }}</span> of {{ $totalJobs }} postings</p>
        </div>
        <div class="table-actions">
            ...
            <select class="filter-select" id="dept-filter" onchange="filterJobs()">
...
{{-- Grid View --}}
<div id="grid-view" style="margin-bottom:22px">
...
{{-- List View --}}
<div id="list-view" style="display:none;margin-bottom:22px">
    <div class="table-section">
        <div class="table-wrapper">
            ...
            <div id="list-empty-state" style="display:none;text-align:center;padding:40px 20px">
                <p style="font-size:13px;color:#9999bb;margin:0">No job postings match your criteria</p>
            </div>
        </div>
        <div class="table-footer">
            <p>Showing <strong id="list-count">{{ $totalJobs }}</strong> of <strong>{{ $totalJobs }}</strong> postings</p>
...
{{-- View Modal --}}
<div class="modal-overlay" id="view-modal" style="display:none" onclick="closeModal('view-modal')">
    <div class="modal-box modal-lg" onclick="event.stopPropagation()">
        ...
        <div class="modal-body">
            ...
            <div class="modal-row"><span>Status</span><span class="badge-status" id="modal-status"></span></div>
        </div>
        <div class="modal-footer">
            <button class="modal-btn-ghost" onclick="closeModal('view-modal')">Close</button>
            <button class="modal-btn-primary" style="background:linear-gradient(135deg,#15803d,#22c55e);box-shadow:0 2px 8px rgba(21,128,61,0.3)">View Applicants</button>
        </div>
    </div>
</div>

{{-- Job Form Modal --}}
<div class="modal-overlay" id="job-form-modal" style="display:none" onclick="closeModal('job-form-modal')">
    <div class="modal-box" onclick="event.stopPropagation()">
        ...
        <div class="modal-body">
            <form id="job-form" method="POST" action="{{ route('recruitments.store') }}">
                @csrf
                <div class="form-grid">
...
function submitJobForm() {
    if (document.getElementById('form-title-input').value && document.getElementById('form-deadline').value) {
        document.getElementById('job-form').submit();
    }
}

['dept-filter','status-filter'].forEach(id => {
...
function filterJobs() {
    ...
    document.querySelectorAll('#list-table-body tr').forEach(row => {
        const match = (dept === 'All Departments' || row.dataset.dept === dept)
                   ...
                   && (!q || row.querySelector('.position-cell').textContent.toLowerCase().includes(q));
    ...
    document.getElementById('list-empty-state').style.display = count === 0 ? 'block' : 'none';
}

document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { closeModal('view-modal'); closeModal('job-form-modal'); }
});
</script>
@endpush
