...
<div class="auth-card" style="max-width:600px">
    <div style="margin-bottom:24px">
        <h2 style="font-size:22px;font-weight:800;color:#0b044d;margin:0 0 6px">Edit Deduction</h2>
        <p style="font-size:13px;color:#6b6a8a;margin:0">Update deduction information</p>
    </div>

    <form action="{{ route('deductions.update', $deduction) }}" method="post" class="auth-form">
        @csrf
        @method('PUT')

        <div class="auth-field">
            <label>Deduction Name <span style="color:#dc2626">*</span></label>
            <input type="text" name="name" value="{{ old('name', $deduction->name) }}" placeholder="e.g. PhilHealth" required>
        </div>

        <div class="auth-field">
            <label>Rate <span style="font-size:11px;color:#9999bb">(leave blank if fixed amount)</span></label>
            <input type="number" name="rate" value="{{ old('rate', $deduction->rate) }}" placeholder="e.g. 0.03 for 3%" step="0.0001" min="0" max="1">
        </div>
        ...
        <div class="auth-field">
            ...
            <select name="type" required>
                ...
                <option value="{{ $ddtype->value }}" {{ old('type', $deduction->type) == $ddtype->value ? 'selected' : '' }}>{{ $ddtype->value }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="auth-field">
            ...
            <textarea name="description" rows="3" style="padding:10px 13px;border:1.5px solid #e0dff5;border-radius:9px;font-size:13.5px;color:#1a1a3a;background:#fafafe;outline:none;width:100%;box-sizing:border-box;font-family:'Poppins',sans-serif;resize:vertical" placeholder="Brief description of this deduction">{{ old('description', $deduction->description) }}</textarea>
        </div>
