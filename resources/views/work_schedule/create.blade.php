...
<div class="auth-card" style="max-width:600px">
    <div style="margin-bottom:24px">
        <h2 style="font-size:22px;font-weight:800;color:#0b044d;margin:0 0 6px">Add Work Schedule</h2>
        <p style="font-size:13px;color:#6b6a8a;margin:0">Create a new employee work schedule</p>
    </div>

    <form action="{{ route('work_schedules.store') }}" method="post" class="auth-form">
        @csrf

        <div class="auth-field">
            <label>Schedule Name <span style="color:#dc2626">*</span></label>
            <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Regular 8-5" required>
        </div>
        ...
        <div class="auth-row-2">
            <div class="auth-field">
                ...
                <input type="time" name="pm_start_time" value="{{ old('pm_start_time') }}" required>
            </div>
            <div class="auth-field">
                <label>Break Minutes <span style="color:#dc2626">*</span></label>
                <input type="number" name="break_minutes" value="{{ old('break_minutes') }}" placeholder="e.g. 60" min="0" required>
            </div>
        </div>

        <div class="auth-field">
            <label>Grace Period (minutes) <span style="color:#dc2626">*</span></label>
            <input type="number" name="grace_period_minutes" value="{{ old('grace_period_minutes') }}" placeholder="e.g. 15" min="0" required>
        </div>
        ...
        <div class="auth-field">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:6px">
                ...
                <label style="display:flex;align-items:center;gap:6px;font-size:13px;font-weight:500;color:#0b044d;cursor:pointer">
                    <input type="checkbox" name="work_days[]" value="{{ $day }}" {{ in_array($day, old('work_days', [])) ? 'checked' : '' }}
